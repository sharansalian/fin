...
export default function Archive() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try   { setArticles(await getArticles(user.uid, { isArchived: true })); }
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ArticleCard
            ...
            onUpdate={(data) => setArticles((prev) => prev.map((a) => a.id === item.id ? { ...a, ...data } : a).filter((a) => a.isArchived))}
          />
        )}
        ...
        ListEmptyComponent={
          <View style={s.empty}>
            ...
            <Text style={s.emptySub}>Swipe left on an article and tap Archive.</Text>
          </View>
        }
